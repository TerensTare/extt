...
public:
    ...
    auto add(duration dt) noexcept {
        timers.push_back({{}, dt, +[](entt::dispatcher &dsp) { dsp.trigger<tag<Tag>>(); }});
    ...
    auto looping(duration dt) noexcept {
        loop_timers.push_back({{}, dt, +[](entt::dispatcher &dsp) { dsp.enqueue<tag<Tag>>(); }});
        return dsp.sink<tag<Tag>>();
    }
    ...
    void update(duration dt) noexcept {
        ...
        auto done = std::remove_if(timers.begin(), timers.end(), [](auto const &t) { return t.elapsed >= t.total; });
